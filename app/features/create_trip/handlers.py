...
Выберите тип поездки 👇""")
    await callback.message.edit_text(text=text, reply_markup=chosse_ts)
...
async def choose_trip_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📍 От какго общежития выезжаем?", reply_markup=chosse_fromP)
...
async def choose_pointF(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📍 Выберите номер учебного здания КАИ (пункт назначения)?", reply_markup=chosse_toP)
...
async def choose_date(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📍 Введите время отправления (в формате HH:MM)\nПример: 09:45", reply_markup=InlineKeyboardMarkup(inline_keyboard=[]))
...
async def choose_places(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Добавьте коментарий к поездке (необязательно)\nНапример место, время сбора или марка личный машны", reply_markup=for_comment)
...
async def confirm_step(message: Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=confirm_create_trip)
...
async def confirm_trip(callback: CallbackQuery, state: FSMContext):
    ...
    success_text = textwrap.dedent("""🎉 Поездка успешно создана!
...
• Примерное время прибытия""")
    ...
    await callback.message.edit_text(success_text, reply_markup=start_keyboard)
    await callback.answer()


@router.callback_query(CreateTrip.confirm, F.data == "change_create_trip")
async def change_create_trip(callback: CallbackQuery, state: FSMContext):
    await state.clear()
